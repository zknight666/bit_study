...
#3. compile, training
model.compile(
    loss='sparse_categorical_crossentropy', # 다중분류 => categorical_crossentropy 사용
    optimizer='adam',
    metrics=['accuracy']
)
...
y_predict=np.argmax(y_predict,axis=1) # argmax 사용한 모양 가장 높은 값 뽑아줌 # 얜 왜씀?
print('y_predict(예측값)',y_predict)
# y_test=np.argmax(y_test,axis=1) # 가장 높은 값 뽑아줌 # onehot 안쓰면 argmax 쓸 필요 없음.
